function [MultiM, vTrain, indx, normP] = mutiview_feature_learning_nn(xTr, yTr, ParaM)
...
for iter =0:30 
    ...
    % 2.
    vtau_hat = vtau_u_n1;
    nablaf_U_hat = U_HAT * V_0 * V_0' - xTrain * V_0';
    f_U_HAT = 0.5*norm(xTrain - U_HAT*V_0,'fro')^2;
    ...
    while (1)
        ...
        F_S_vtau_hat_G_U = 0.5*norm(xTrain - S_vtau_hat_G_U*V_0,'fro')^2 + alpha1*S_vtau_hat_G_U_21;
    ...
    % 2.
    vtau_hat = vtau_v_n1;
    nablaf_V_hat = -U_0' * xTrain +  U_0' * U_0 * V_HAT + alpha2 * V_HAT + alpha3 * V_HAT * L;
    f_V_HAT = 0.5*norm(xTrain - U_0*V_HAT,'fro')^2 + 0.5*alpha2*norm(V_HAT,'fro')^2 + 0.5*alpha3*trace(V_HAT*L*V_HAT');
    nablaf_V_hat_norm = norm(nablaf_V_hat,'fro')^2;
    while (1)
        G_V = V_HAT - nablaf_V_hat/vtau_hat;
        S_vtau_hat_G_V = G_V;
        F_S_vtau_hat_G_V = 0.5*norm(xTrain - U_0*S_vtau_hat_G_V,'fro')^2 + 0.5*alpha2*norm(S_vtau_hat_G_V,'fro')^2 + 0.5*alpha3*trace(S_vtau_hat_G_V*L*S_vtau_hat_G_V');
